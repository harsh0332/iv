...
import { ShieldCheck, MapPin, Landmark, Milestone, Camera } from "lucide-react";
...
export default function TrustBar() {
  const trustCards = [
    ...
    },
    {
      id: "tncp",
      title: "TNCP Approved",
      value: "Bhopal TNCP Layout Plan",
      icon: <MapPin className="w-5 h-5 text-secondary-500" />,
    },
    {
      id: "plots",
      title: "10-Acre Gated Layout",
      value: "125 Planned Plots",
    ...
    },
    {
      ...
      value: "Verified Ground Views",
      icon: <Camera className="w-5 h-5 text-secondary-500" />,
    },
  ];

  return (
    <section className="relative z-10 w-full bg-white border-b border-b-border-soft/60 py-6 font-sans">
      <Container>
            ...
            if (card.link) {
              return (
                <motion.a
                  ...
                  className="flex-shrink-0 w-[240px] sm:w-[280px] lg:w-auto snap-center flex items-center gap-3.5 bg-accent/30 lg:bg-transparent p-4 lg:p-0 rounded-xl lg:rounded-none border border-border-soft/40 lg:border-none hover:opacity-85 transition-opacity"
                >
                  {content}
                </motion.a>
              );
        ...
        {/* Honest Status Line */}
        <div className="text-center mt-6 pt-4 border-t border-border-soft/60 text-xs text-text-main/70 font-semibold font-sans">
          Project status: RERA-registered, under active development — visit to see live progress.
        </div>
      </Container>
    </section>
  );
